...
void CollisionsBuilder::add_vertex_vertex_collisions(
    ...
    const size_t end_i)
{
    for (size_t i = start_i; i < end_i; i++) {
        ...
        VertexVertexCollision vv(vi, vj, weight, weight_gradient);
        vv_to_id.emplace(vv, vv_collisions.size());
        vv_collisions.push_back(vv);
    }
}

void CollisionsBuilder::add_edge_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
...
void CollisionsBuilder::add_edge_vertex_collision(
    const CollisionMesh& mesh,
...
void CollisionsBuilder::add_edge_edge_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
...
void CollisionsBuilder::add_face_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
...
void CollisionsBuilder::add_edge_vertex_negative_vertex_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
    const std::vector<VertexVertexCandidate>& candidates,
    const size_t start_i,
...
void CollisionsBuilder::add_face_vertex_positive_vertex_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
    const std::vector<VertexVertexCandidate>& candidates,
    const size_t start_i,
...
void CollisionsBuilder::add_face_vertex_negative_edge_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
    const std::vector<EdgeVertexCandidate>& candidates,
    const size_t start_i,
...
void CollisionsBuilder::add_edge_edge_negative_edge_vertex_collisions(
    const CollisionMesh& mesh,
    const Eigen::MatrixXd& vertices,
    const std::vector<EdgeVertexCandidate>& candidates,
    const size_t start_i,
...
void CollisionsBuilder::add_vertex_vertex_collision(
...
void CollisionsBuilder::add_edge_edge_collision(
    ...
    Collisions& merged_collisions)
{
    ...
    vv_collisions.erase(
        std::remove_if(
            ...
            [&](const VertexVertexCollision& vv) { return vv.weight == 0; }),
        vv_collisions.end());
    // Same for edge-vertex collisions.
    ev_collisions.erase(
        std::remove_if(
            ev_collisions.begin(), ev_collisions.end(),
            [&](const EdgeVertexCollision& ev) { return ev.weight == 0; }),
        ev_collisions.end());
    // Same for edge-edge collisions.
    ee_collisions.erase(
        std::remove_if(
            ee_collisions.begin(), ee_collisions.end(),
            [&](const EdgeEdgeCollision& ee) { return ee.weight == 0; }),
        ee_collisions.end());
}
